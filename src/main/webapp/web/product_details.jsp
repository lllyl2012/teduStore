...
<nav id="nav">
    <ul>
        ...
        <li><a href="${base}/goods/list.do?id=${goodsCategorySecond.id}" >${goodsCategorySecond.name}</a></li>
        <li><a href="${base}/goods/list.do?id=${goodsCategoryThird.id}" >${goodsCategoryThird.name}</a></li>
    </ul>
</nav>
<!-- 内容-->
<!--细节导航-->
<div id="nav_detail">
    <h5><a href="${base}/main/index.do">首页</a> > 
    <a href="${base}/goods/list.do?id=${goodsCategoryOne.id}" >${goodsCategoryOne.name}</a>
    ><a href="${base}/goods/list.do?id=${goodsCategorySecond.id}" >${goodsCategorySecond.name}</a> 
    > <a href="${base}/goods/list.do?id=${goodsCategoryThird.id}" >${goodsCategoryThird.name}</a>
     >${goods.itemType}></h5>
</div>
<!--产品预览-->
<div id="shop_detail">
    ...
    <div id="preview" class="lf">
        <div id="mediumDiv">
            <img id="mImg" src="${base}${goods.image}"/>
        </div>
        <div id="icon_all">
            <ul id="icon_list">
    ...
    <div class="right_detail lf">
        ...
        <p id="goods_list">
            <s>规格：</s><br>
            	<c:forEach var="i" items="${goodsList}">
		            <a class="goods_title
		            	<c:if test="${i.id eq goods.id}">
		            					selected
		            	</c:if>
		            
		            " id="${i.id}" href="${base}/goods/detail.do?id=${i.id}">${i.title}</a>
            	</c:forEach>
        </p>
        <!-- 数量-->
        ...
        <div class="shops">
            ...
            <a  id="addCart" class="shop lf"><img src="../images/product_detail/product_detail_img7.png"  alt=""/>加入购物车</a>
...
<script type="text/javascript">
	$(function(){
		$("#addCart").click(function(){
			var num = $("#buy-num").val();
			var goodsImage = "${goods.image}";
			var goodsPrice = "${goods.price}";
			var goodsId = "${goods.id}";
			var goodsTitle = "${goods.title}";
			var url = "${base}/cart/add.do";
			console.log(url+num+goodsImage+goodsPrice+goodsId+goodsTitle);
			$.ajax({
				"url":url,
				"data":{"num":num,"goodsImage":goodsImage,"goodsPrice":goodsPrice,"goodsId":goodsId,"goodsTitle":goodsTitle},
				"dataType":"json",
				"type":"post",
				"success":function(o){
					alert(o.message);
